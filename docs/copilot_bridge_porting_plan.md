...
- P2.2.2 附件输入约束
  - 识别飞书附件 / 图片 / 富文本里的非纯文本输入，并给出清晰的约束说明或降级路径
  - 为后续截图、日志片段、补丁片段输入预留统一入口
- P2.2.3 审批轨迹增强
  - 让批准 / 拒绝 / 重试这类关键动作在本地留下更完整的审计轨迹
  - 明确失败、拒绝、成功三类状态的可回溯记录
...
后续如果上下文丢失，直接从本文件按 `Suggested Build Order` 继续实现。`P2.1.4` 已完成，当前已进入 `P2.2`，并已落地 `P2.2.1` 的会话隔离与基础审计；下一步优先继续附件 / 多模态输入约束与处理路径。